"""
OFFICE SURVIVAL KIT — Blender 3D Model Generator (v3: Dimension Corrections)
=============================================================================
Paste and run in Blender > Scripting workspace.
Scale: 1 Blender Unit = 1 inch.

v3 changes (March 11, 2026):
  - Fixed back zone foam padding: split symmetric 0.25" into
  ...
  - Fixed Yard Pass dimensions: 4.0" × 2.5" (was 3.5" × 2.5")
...
# Booklet thicknesses (per Dimensions Spec)
...
# ── BACK ZONE STACK (per Dimensions Spec — asymmetric foam) ────────────────
BACK_YC       = (BACK_Y0 + BACK_Y1) / 2           # -3.375
FOAM_BOTTOM_T = 0.50                               # Bottom foam — drop protection
FOAM_TOP_T    = 0.125                              # Top foam — rattle prevention
...
foam_h = Z_FOAM_TOP - Z_FOAM_BASE                  # 4.125"
box("Foam_Padding",
...
# Yard Pass — flanking right (4.0" × 2.5" × 0.10" per Dimensions Spec)
box("Item_Yard_Pass",
    (4.25, BACK_YC, Z_MUG_BOT + 0.05),
    (4.0, 2.5, 0.10), M_ITEM)
...
# Turf Tags — 4.5" × 1.25" per Dimensions Spec (was 4.0")
box("Item_Turf_Tags",
    (4.00, TRAY_YC + 1.50, t2z + 0.050),
...
# Sticker Guide Card — ~3.0" × ~4.0" per Dimensions Spec (was 2.5" × 2.0")
box("Item_Sticker_Guide_Card",
    (5.50, TRAY_YC - 0.75, t2z + 0.050),
    (3.0, 4.0, 0.10), M_ITEM)
...
# Rap Sheet — 4.0" × 5.5" per Dimensions Spec (was 4.0" × 4.5")
box("Item_Rap_Sheet_Notepad",
    (2.00, TRAY_YC + 0.25, t1z + 0.0625),
    (4.0, 5.5, 0.125), M_ITEM)

# Tattoo Guide Card — ~3.0" × ~4.0" per Dimensions Spec (was 3.0" × 2.0")
...
print("=" * 66)
print("  OFFICE SURVIVAL KIT — Blender Model v3 (Dimension Corrections)")
print("=" * 66)
...
print(f"  ───────────────────────────────────────────")
print(f"  BACK ZONE STACK (per Dimensions Spec):")
print(f"    Foam bottom:        {Z_FOAM_BASE:.3f}\" → {Z_MUG_BOT:.3f}\"  ({FOAM_BOTTOM_T}\" thick)")
print(f"    Mug (on side):      {Z_MUG_BOT:.3f}\" → {Z_MUG_TOP:.3f}\"  (3.5\" dia)")
print(f"    Foam top:           {Z_MUG_TOP:.3f}\" → {Z_FOAM_TOP:.3f}\"  ({FOAM_TOP_T}\" thick)")
