"""
OFFICE SURVIVAL KIT — Blender 3D Model Generator (v4: Ideation Cascade)
=============================================================================
Paste and run in Blender > Scripting workspace.
Scale: 1 Blender Unit = 1 inch.

v4 changes (March 11, 2026):
  - Booklets → binder-clipped document sets (material + naming updated)
  - Die-cut foam → shredded "CLASSIFIED" paper fill (material updated)
  - Yard Pass removed (merged into Corporate Parole Passport)
  - Tray 2: 7 individual items → 5 tray positions
    (Cry Rags, Turf Tags, Sticker Guide Card, Evidence Bag, Empty Stapler Recess)
  - Added Evidence Bag recess and Confiscated Stapler empty recess
  - Booking frame comment updated (rests on Document Set 1)
  - Print summary updated for new item counts

v3 changes (March 11, 2026):
  - Fixed back zone padding: split symmetric 0.25" into
    bottom 0.50" / top 0.125" per Dimensions Spec
  - Fixed mug Z position (center now at Z=2.50", was 2.25")
  - Fixed Passport dimensions: 5.0" × 3.5" (was 3.5" × 2.5")
  - Fixed Rap Sheet dimensions: 4.0" × 5.5" (was 4.0" × 4.5")
  - Fixed Tattoo Guide Card: 3.0" × 4.0" (was 3.0" × 2.0")
  - Fixed Turf Tags: 4.5" × 1.25" (was 4.0" × 1.25")
  - Fixed Sticker Guide Card: 3.0" × 4.0" (was 2.5" × 2.0")
  - Adjusted item positions to avoid clipping with corrected sizes
  - Back zone stack now matches Dimensions Spec (~4.69" total)
...
LAYOUT — Two-zone side-by-side interior:
  ...
  BACK ZONE (-Y side, ~4.25" deep):
    ...
    Corporate Parole Passport (includes Yard Pass pages).
  ...
  FRONT ZONE (+Y side, ~6.5" deep):
    Stack (bottom to top):
      Tray 2 (5 positions) → Doc Set 2 → Tray 1 (4 items) →
      Doc Set 1 → Booking Frame → [headroom] → Lid
    Trays use die-cut recesses (Pelican case style) with
    chalk-outline silhouettes, item name + tagline per recess.
    Document sets are binder-clipped stacks, ~13" × 6.5".
...
Reveal sequence:
  ...
  Doc Set 1 → Tray 1 (4 items) → Doc Set 2 →
  Tray 2 (5 positions: 3 items + evidence bag + empty stapler recess) →
  Compartment cover → Mug + Passport in shredded paper → Base message
"""
...
M_CLIP      = make_mat("Binder_Clip",      (0.02, 0.02, 0.02), rough=0.30, metal=0.8)
M_EVIDENCE  = make_mat("Evidence_Bag",     (0.85, 0.85, 0.80), rough=0.60)
...
TRAY_L  = INT_L            # 13.5"  (~13.5" exterior per Dimensions Spec)
TRAY_W  = FRONT_ZONE_D     # 6.5"   (~6.5" exterior per Dimensions Spec)
...
T2_WH = 0.875              # Tray 2 wall height (5 tray positions)
T1_H  = TRAY_FT + T1_WH    # 0.875" total
T2_H  = TRAY_FT + T2_WH    # 1.000" total

# Document set thicknesses (per Dimensions Spec — same as old booklet dims)
DS1_T = 0.25               # Document Set 1 (intake, binder-clipped)
DS2_T = 0.30               # Document Set 2 (escalation, binder-clipped)
...
Z_DS2_TOP   = Z_DS2 + DS2_T                       # 1.550"
Z_T1        = Z_DS2_TOP                            # 1.550"  Tray 1 bottom
Z_T1_TOP    = Z_T1 + T1_H                         # 2.425"
Z_DS1       = Z_T1_TOP                             # 2.425"  Doc Set 1 bottom
Z_DS1_TOP   = Z_DS1 + DS1_T                       # 2.675"
Z_FRAME     = Z_DS1_TOP                            # 2.675"  Booking frame bottom
...
PAPER_BOTTOM_T = 0.50                              # Bottom padding — drop protection
PAPER_TOP_T    = 0.125                             # Top padding — rattle prevention
...
Z_MUG_BOT     = Z_PAPER_BASE + PAPER_BOTTOM_T     # 0.750"
Z_MUG_TOP     = Z_MUG_BOT + MUG_DIA               # 4.250"
Z_PAPER_TOP   = Z_MUG_TOP + PAPER_TOP_T           # 4.375"

# Compartment cover
COVER_L = 13.5              # ~INT_L
COVER_W = 4.0               # Spans most of back zone
COVER_T = 0.0625            # ~24pt rigid paperboard
Z_COVER = Z_PAPER_TOP                              # 4.375"
...
# Shredded "CLASSIFIED" paper fill (replaces die-cut foam from v3)
paper_h = Z_PAPER_TOP - Z_PAPER_BASE               # 4.125"
...
mug_z = Z_MUG_BOT + MUG_DIA / 2                    # Center at 2.50"
...
# Corporate Parole Passport — flanking left (5.0" × 3.5" × 0.25" per Dimensions Spec)
# Now includes Yard Pass pages (12-16 total pages). No separate Yard Pass.
...
# ── TRAY 2 — Arsenal (bottom tray, 5 tray positions, die-cut recesses) ────
tray("Tray_2_Arsenal", TRAY_L, TRAY_W, T2_WH, TRAY_FT,
     tray_origin(Z_T2), M_CHARCOAL)
t2z = Z_T2 + TRAY_FT

# Position 1: Contraband Cry Rags (tissues) — 3.0" × 3.0" × 0.75"
box("Item_Cry_Rags",
    (-4.50, TRAY_YC + 1.50, t2z + 0.375),
    (3.0, 3.0, 0.75), M_ITEM)

# Position 2: Turf Tags (stickers) — 4.5" × 1.25" per Dimensions Spec
box("Item_Turf_Tags",
    (-0.50, TRAY_YC + 1.50, t2z + 0.050),
    (4.5, 1.25, 0.10), M_ITEM)

# Position 3: Sticker Guide Card — ~3.0" × ~4.0" per Dimensions Spec
box("Item_Sticker_Guide_Card",
    (-0.50, TRAY_YC - 1.00, t2z + 0.050),
    (3.0, 4.0, 0.10), M_ITEM)

# Position 4: "CONFISCATED ITEMS" Evidence Bag — ~4" × 5" zip-lock bag
# Contains 4 items inside: Snitch Switch, Evidence Eraser, Solitary, Ankle Monitor
box("Item_Evidence_Bag",
    (4.00, TRAY_YC + 0.50, t2z + 0.250),
    (4.0, 5.0, 0.50), M_EVIDENCE)

# Position 5: Empty Confiscated Stapler Recess — chalk outline only, NO item
# "CONFISCATED — WEAPONIZED OFFICE EQUIPMENT. INCIDENT #147. DO NOT REISSUE."
# Represented as a thin orange outline on the tray floor (the absence IS the joke)
box("Recess_Confiscated_Stapler",
    (-4.50, TRAY_YC - 1.50, t2z + 0.005),
    (3.5, 1.5, 0.01), M_ORANGE)


# ── DOCUMENT SET 2 — Escalation / Incident (on Tray 2) ──────────────────────
box("DocSet_2_Escalation",
    (TRAY_XC, TRAY_YC, Z_DS2 + DS2_T / 2),
    (13.0, 6.5, DS2_T), M_DOCSET)

# Binder clip on Document Set 2
box("DocSet_2_BinClip",
    (TRAY_XC - 6.0, TRAY_YC, Z_DS2 + DS2_T + 0.10),
    (1.0, 0.50, 0.20), M_CLIP)
...
# Rap Sheet — 4.0" × 5.5" per Dimensions Spec
# Each page is a unique incident report form (8-10 designs)
box("Item_Rap_Sheet_Notepad",
    (2.00, TRAY_YC + 0.25, t1z + 0.0625),
    (4.0, 5.5, 0.125), M_ITEM)

# Tattoo Guide Card — ~3.0" × ~4.0" per Dimensions Spec
box("Item_Tattoo_Guide_Card",
    (5.25, TRAY_YC - 0.50, t1z + 0.05),
    (3.0, 4.0, 0.10), M_ITEM)


# ── DOCUMENT SET 1 — Intake / Orientation (on Tray 1) ───────────────────────
box("DocSet_1_Intake",
    (TRAY_XC, TRAY_YC, Z_DS1 + DS1_T / 2),
    (13.0, 6.5, DS1_T), M_DOCSET)

# Binder clip on Document Set 1
box("DocSet_1_BinClip",
    (TRAY_XC - 6.0, TRAY_YC, Z_DS1 + DS1_T + 0.10),
    (1.0, 0.50, 0.20), M_CLIP)
...
print("=" * 66)
print("  OFFICE SURVIVAL KIT — Blender Model v4 (Ideation Cascade)")
print("=" * 66)
...
print(f"    Tray 2:             {Z_T2:.3f}\" → {Z_T2_TOP:.3f}\"  (5 positions)")
print(f"    Doc Set 2:          {Z_DS2:.3f}\" → {Z_DS2_TOP:.3f}\"  (0.30\" thick)")
print(f"    Tray 1:             {Z_T1:.3f}\" → {Z_T1_TOP:.3f}\"  (4 items)")
print(f"    Doc Set 1:          {Z_DS1:.3f}\" → {Z_DS1_TOP:.3f}\"  (0.25\" thick)")
...
print(f"    Paper bottom:       {Z_PAPER_BASE:.3f}\" → {Z_MUG_BOT:.3f}\"  ({PAPER_BOTTOM_T}\" thick)")
print(f"    Mug (on side):      {Z_MUG_BOT:.3f}\" → {Z_MUG_TOP:.3f}\"  (3.5\" dia)")
print(f"    Paper top:          {Z_MUG_TOP:.3f}\" → {Z_PAPER_TOP:.3f}\"  ({PAPER_TOP_T}\" thick)")
...
print(f"  Mug center Z:        {mug_z:.3f}\" (should be ~2.50\")")
...
print(f"  Booking frame:        Separate removable object, back is copy surface ✓")
...
print(f"  Tray 2 positions:     5 (tissues, stickers, sticker guide, evidence bag, empty stapler) ✓")
print(f"  Compartment items:    2 (mug, passport — Yard Pass merged into passport) ✓")
print(f"  Document sets:        2 (binder-clipped, NOT coil-bound) ✓")
print(f"  Back zone fill:       Shredded CLASSIFIED paper (NOT die-cut foam) ✓")
